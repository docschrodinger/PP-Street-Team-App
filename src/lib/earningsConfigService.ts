...
import { getEarningsConfig, calculateEstimatedMonthlyEarnings, calculateEstimatedAnnualEarnings } from '../lib/earningsConfigService';
import type { StreetUser } from '../lib/types';
import type { EarningsConfig } from '../lib/earningsConfigService';
...
export function EarningsScreen({ user, onBack }: EarningsScreenProps) {
  ...
  async function loadEarningsData() {
    // Load earnings configuration from database
  ...
  // Uses database-driven configuration instead of hardcoded values
  const estimatedMonthly = earningsConfig 
    ...
    : 0;
  const estimatedAnnual = earningsConfig 
    ? calculateEstimatedAnnualEarnings(liveVenues.length, user.current_rank, earningsConfig)
    : 0;
  const potentialMonthly = earningsConfig
    ? calculateEstimatedMonthlyEarnings(liveVenues.length + pendingVenues.length, user.current_rank, earningsConfig)
    : 0;
  const totalPending = earningsConfig
    ? calculateEstimatedMonthlyEarnings(pendingVenues.length, user.current_rank, earningsConfig)
    : 0;
